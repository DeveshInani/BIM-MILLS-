...
export default function Login({ mode = 'light' }) {
    ...
    return (
        <div className={`min-h-screen flex items-center justify-center py-10 sm:py-16 px-1 sm:px-4 transition-colors duration-300 ${darkMode ? 'bg-slate-950' : 'bg-gray-50'
            }`}>
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className={`w-full max-w-md p-5 sm:p-8 rounded-[1.75rem] sm:rounded-3xl shadow-2xl ${darkMode ? 'bg-gray-900 border border-white/10' : 'bg-white'
                    }`}
            >
