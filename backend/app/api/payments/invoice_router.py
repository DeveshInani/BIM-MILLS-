...
from app.db.models import Invoice, Order, User, Admin
...
def generate_invoice(
    ...
    """Generate an invoice (Ownership check required)"""
    # Fetch the order
...
def list_invoices(
    ...
    """List all invoices (Admin sees all, User sees only their own)"""
    query = db.query(Invoice).order_by(Invoice.created_at.desc())
    
    if auth["role"] == "user":
        query = query.filter(Invoice.customer_email == auth["auth"].email)
...
def get_invoice(
    ...
    """Get a specific invoice by ID (Ownership check)"""
    invoice = db.query(Invoice).filter(Invoice.id == invoice_id).first()
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
        
    if auth["role"] == "user" and invoice.customer_email != auth["auth"].email:
...
def get_invoices_by_order(
    ...
    """Get invoices for a specific order (Ownership check)"""
    order = db.query(Order).filter(Order.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
        
    if auth["role"] == "user" and order.user_email != auth["auth"].email:
        raise HTTPException(status_code=403, detail="Not authorized to view invoices for this order")
