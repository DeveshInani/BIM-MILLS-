...
def cancellation_request_received_template(name, order_id):
    return f"""
    <html>
      <body style="font-family:Arial;background:#f4f6f8;padding:30px;">
        <div style="max-width:600px;margin:auto;background:white;border-radius:10px;padding:30px;">
          <h2 style="color:#1976d2;">Cancellation Request Received</h2>
          <p>Dear <strong>{name}</strong>,</p>
          <p>We have received your request to cancel order <strong>#{order_id}</strong>.</p>
          <p>Our admin team is reviewing your request. You will receive an update shortly regarding the status of your cancellation.</p>
          <p>Regards,<br><strong>BIM Mills Team</strong></p>
        </div>
      </body>
    </html>
    """

def admin_cancellation_alert_template(order_id, user_name, product_name):
    return f"""
    <html>
      <body style="font-family:Arial;background:#f4f6f8;padding:30px;">
        <div style="max-width:600px;margin:auto;background:white;border-radius:10px;padding:30px;">
          <h2 style="color:#d32f2f;">Action Required: Cancellation Request</h2>
          <p>A new cancellation request has been submitted for Order <strong>#{order_id}</strong>.</p>
...
def cancellation_rejected_template(name, order_id, note):
    return f"""
    <html>
      <body style="font-family:Arial;background:#f4f6f8;padding:30px;">
        <div style="max-width:600px;margin:auto;background:white;border-radius:10px;padding:30px;">
          <h2 style="color:#f44336;">Cancellation Request Update</h2>
          <p>Dear <strong>{name}</strong>,</p>
          <p>Your request to cancel order <strong>#{order_id}</strong> has been reviewed.</p>
          <p><strong>Status:</strong> <span style="color:#f44336;font-weight:bold;">REJECTED</span></p>
          <div style="margin:20px 0;padding:15px;background:#fffde7;border-left:4px solid #fdd835;">
